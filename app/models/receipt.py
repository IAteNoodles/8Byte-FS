...
class ReceiptData(BaseModel):
    ...
    transaction_date: date = Field(
        ..., description="Date of the transaction or start of the billing cycle"
    )
    ...
    upload_timestamp: datetime = Field(
        ...
        description="Timestamp when the receipt was updated",
    )
    currency: str = Field(
    ...
    def has_valid_extentions(cls, v: str) -> str:
        ...
        if not v.lower() in allowed_extensions:
            raise ValueError(
                f"{v} is not valid. Valid extensions include: {', '.join(allowed_extensions)}"
            )
